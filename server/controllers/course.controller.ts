  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await NotificationModel.create({
        user: req.user?._id,
        title: "New Question",
        message: `A new question has been added to ${courseContent.title}`,
      });
    ...
    try {
      ...
      if (req.user?._id === question.user._id) {
        await NotificationModel.create({
          user: req.user?._id,
          title: "New Answer",
          message: `A new reply has been added to ${courseContent.title}`,
        });
      } else {
        const data = {
        ...
        const html = await ejs.renderFile(
          path.join(__dirname, "../mails/questin-reply.ejs"),
          data
        );
        try {
          await sendMail({
